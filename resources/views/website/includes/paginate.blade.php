...
@if($last_page > 1)
    <ul class="uk-pagination uk-flex-center" uk-margin>
        <li class="{{ ($current_page == 1) ? ' uk-disabled' : '' }}">
            <a href="{{ $data->url(($current_page-1) > 0 ? $current_page-1 : $current_page) }}"><span uk-pagination-previous></span></a>
        </li>
        ...
        @if ($last_page <= 10)
            ...
            <li class="{{ $i == $current_page ? 'uk-active' : '' }}">
                <a {{ ($current_page == $i) ? '' : "href=" . $data->url($i) }}>{{ $i }}</a>
            </li>
            @endfor
        ...
        <li class="{{ ($current_page == $last_page) ? ' uk-disabled' : '' }}">
            <a href="{{ $data->url($current_page+1) }}"> <span uk-pagination-next></span></a>
        </li>
    </ul>
@endif
