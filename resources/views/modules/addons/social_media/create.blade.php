...
<div class="container-fluid">
    <form class="m-t-20" role="form" method="POST" action="{{ route('social.media.store') }}">
        @csrf
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Create Social Media 
                            ...
                            <div class="form-input-group">
                                ...
                                <select name="social" class="form-control @error('social') error @enderror" required>
                                    ...
                                    @forelse ($socials as $key => $social)
                                        <option value="{{ $key }}" @if(old('social')==$key) selected @endif>{{ ucwords($key) }}</option>
                                    @empty
                                    @endforelse
                                </select>
                                @error('social')
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-6 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    CREATE <span class="visible-x-inline m-l-5">SOCIAL MEDIA</span>
                                </button>
                            </div>
                        </div>
                    </div>
