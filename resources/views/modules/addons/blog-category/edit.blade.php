...
<div class="container-fluid">
    <form class="m-t-20" role="form" method="POST" action="{{ route('blog.category.update', [$category->uuid]) }}">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update Blog Category
                    ...
                    <div class="card-body">
                        <div class="form-group required form-group-default @error('name') has-error @enderror">
                            ...
                            <div class="controls">
                                <input class="form-control @error('name') error @enderror" name="name" value="{{ $category->name ?? old('name') }}" required>
                                @error('name')
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="form-check info m-b-10">
                                    <input type="checkbox" name="is_active" value="{{ $category->is_active }}" id="checkbox-active-1" @if($category->is_active == 10) checked @endif>
                                    <label for="checkbox-active-1">Publish ?</label>
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6 col-lg-6 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    UPDATE <span class="visible-x-inline m-l-5">BLOG CATEGORY</span>
                                </button>
                            </div>
                        </div>
                    </div>
