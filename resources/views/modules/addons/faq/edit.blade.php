...
<div class="container-fluid">
    <form role="form" class="m-t-20 has-hyperlink-search" method="POST" action="{{ route('faq.update', [$faq->uuid]) }}"
            ...
            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update FAQ 
                    ...
                    <div class="card-body">
                        <div class="form-group required form-group-default @error('question') has-error @enderror">
                            ...
                            <div class="controls">
                                <textarea class="form-control @error('question') error @enderror" name="question" required style="height:100px;">{{ $faq->question ?? old('question') }}</textarea>
                                @error('question')
                        ...
                        <div class="editor-description parent-section m-b-10 @error('answer') has-error @enderror" data-index="{{ $indexing }}">
                            <textarea class="editor-container editor-container-{{ $indexing }} @error('answer') error @enderror" name="answer">{{ $faq->answer ?? old('answer') }}</textarea>
                            @error('answer')
                            <label class="error">{{ $message }}</label>
                            @enderror
                        </div>
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="form-check info m-b-0">
                                    <input type="checkbox" name="is_active" value="10" id="checkbox-active" @if($faq->is_active == 10) checked @endif>
                                    <label for="checkbox-active">Publish ?</label>
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6 col-lg-6 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    UPDATE <span class="visible-x-inline m-l-5">FAQ</span>
                                </button>
                            </div>
                        </div>
                    </div>
