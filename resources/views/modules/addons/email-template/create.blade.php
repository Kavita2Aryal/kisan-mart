...
<div class="container-fluid">
    <form role="form" class="m-t-20" method="POST" action="{{ route('email.template.store') }}">
        @csrf
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Create Email Template 
                    ...
                    <div class="card-body">
                        <div class="form-group required form-group-default @error('title') has-error @enderror">
                            <label>Title</label>
                            <div class="controls">
                                <input type="text" class="form-control @error('title') error @enderror" name="title" value="{{ old('title') }}">
                                @error('title')
                        ...
                        <div class="form-group form-group-default @error('hint') has-error @enderror">
                            <label>Hint</label>
                            <div class="controls">
                                <textarea class="form-control @error('hint') error @enderror" name="hint">{{ old('hint') }}</textarea>
                                @error('hint')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
                        ...
                        <div class="editor-description parent-section m-b-10 @error('template') has-error @enderror" data-index="{{ $indexing }}">
                            <textarea class="editor-container editor-container-{{ $indexing }} @error('template') error @enderror" name="template">{!! old('template') !!}</textarea>
                            @error('template')
                            <label class="error">{{ $message }}</label>
                            @enderror
                        </div>
                        <div class="row">
                            <div class="col-sm-12 col-md-12 col-lg-12 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    CREATE <span class="visible-x-inline m-l-5">EMAIL TEMPLATE</span>
                                </button>
                            </div>
                        </div>
                    </div>
