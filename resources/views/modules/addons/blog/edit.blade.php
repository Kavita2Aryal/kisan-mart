...
<div class="container-fluid">
    <form class="m-t-20 blog-form-fix has-hyperlink-search" id="blog-form" role="form" method="POST" action="{{ route('blog.update', [$blog->uuid]) }}" data-generate="{{ route('blog.generate.form') }}" data-check="{{ route('blog.check', [$blog->uuid]) }}">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-8 col-xlg-8">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update Blog 
                            <button class="btn btn-link btn-link-fix p-l-10 p-r-10 m-l-5 m-r-5 pull-right" type="submit">
                                UPDATE <span class="visible-x-inline m-l-5">BLOG</span>
                            </button>
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                                <div class="form-group required form-group-default @error('title') has-error @enderror">
                                    <label>Title</label>
                                    <textarea class="form-control alias-source @error('title') error @enderror" name="title" required style="height: 45px;">{{ $blog->title ?? old('title') }}</textarea>
                                    @error('title')
                                ...
                                <div class="form-group form-group-default required input-group alias-edit @error('alias') has-error @enderror">
                                    ...
                                    <div class="form-input-group">
                                        <input type="text" class="form-control alias-value alias-check p-b-5 @error('alias') error @enderror" name="alias" placeholder="Web Alias" required autocomplete="off" value="{{ $blog->alias->alias ?? old('alias') }}">
                                        <input type="hidden" class="alias-index" name="alias_id" value="{{ $blog->alias->id }}">
                                    </div>
                                </div>
                                @error('alias')
                                ...
                                <div class="form-group form-group-default @error('subtitle') has-error @enderror">
                                    ...
                                    <textarea class="form-control @error('subtitle') error @enderror" name="subtitle" style="height: 45px;">{{ $blog->subtitle ?? old('subtitle') }}</textarea>
                                    @error('subtitle') 
                                    <label class="error">{{ $message }}</label>
                                    @enderror
                                </div>
                                ...
                                <div class="blog-container content-container">
                                    @php $index = 1; @endphp
                                    @forelse ($blog->contents as $content)

                                        @if ($content->display_type == 2)

                                        @include('includes.addons-image-gallery-form', ['index' => $index, 'content' => $content])

                                        @elseif ($content->display_type == 1)

                                        @include('includes.addons-description-form', ['index' => $index, 'content' => $content])
                    ...
                    <div class="card-body">
                        <div class="form-group form-group-default input-group required @error('published_at') has-error @enderror">
                            <div class="form-input-group">
                                ...
                                <input type="text" class="form-control published-date @error('published_at') error @enderror" name="published_at" placeholder="Pick a date" autocomplete="off" required value="{{ $blog->published_at ?? old('published_at') }}" data-provide="datepicker-inline">
                                @error('published_at')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
                        ...
                        <div class="form-group form-group-default form-group-tagsinput blog-keywords @error('keywords') has-error @enderror" style="height:100px;">
                            ...
                            <input type="text" class="form-control @error('keywords') error @enderror ignore-navigate-away" name="keywords" placeholder="Search Keywords" autocomplete="off" value="{{ $blog->keywords ?? old('keywords') }}">
                            @error('keywords')
                            <label class="error">{{ $message }}</label>
                            @enderror
                        </div>
                        <div class="form-check info m-b-10">
                            <input type="checkbox" name="is_active" value="{{ $blog->is_active }}" id="checkbox-active-1" @if($blog->is_active == 10) checked @endif>
                        ...
                        <div class="row m-t-15">
                            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                                ...
                                <label class="error">{{ $message }}</label>
                                @enderror
                                <input type="hidden" name="intro_image_id" class="populate-value-intro" value="{{ $blog->intro_image_id}}">                                
                                <div class="populate-container populate-container-intro m-t-10">
                                    @if (isset($blog->intro_image_id) && isset($blog->intro_image->image))
                                    <div class="populate-media">
                                        <a href="{{ secure_img($blog->intro_image->image, '1200') }}" target="_blank">
                                            <img src="{{ secure_img($blog->intro_image->image, '480X320') }}" class="full-width">
                                        </a>
                                    </div>
                                    @endif
                                </div> 
                        ...
                        <div class="row m-t-10">
                            <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                                ...
                                <label class="error">{{ $message }}</label>
                                @enderror
                                <input type="hidden" name="banner_image_id" class="populate-value-banner" value="{{ $blog->banner_image_id}}">
                                <div class="populate-container populate-container-banner m-t-10">
                                    @if (isset($blog->banner_image_id) && isset($blog->banner_image->image))
                                    <div class="populate-media">
                                        <a href="{{ secure_img($blog->banner_image->image, '1200') }}" target="_blank">
                                            <img src="{{ secure_img($blog->banner_image->image, '480X320') }}" class="full-width">
                                        </a>
                                    </div>
                                    @endif
                                </div> 
                    ...
                    <div class="card-body">
                        <div class="seo-content">
                            <div class="form-group required form-group-default @error('seo.meta_title') has-error @enderror">
                                ...
                                <textarea class="form-control @error('seo.meta_title') error @enderror" name="seo[meta_title]" placeholder="Meta Title" required style="height:40px;">{{ $blog->seo->meta_title ?? old('seo.meta_title') }}</textarea>
                            ...
                            <div class="form-group required form-group-default @error('seo.meta_description') has-error @enderror">
                                ...
                                <textarea class="form-control @error('seo.meta_description') error @enderror" name="seo[meta_description]" placeholder="Meta Description" required style="height:60px;">{{ $blog->seo->meta_description ?? old('seo.meta_description') }}</textarea>
                            ...
                            <div class="form-group required form-group-default form-group-tagsinput seo-keywords @error('seo.meta_keywords') has-error @enderror" style="height:100px;">
                                ...
                                <input type="text" class="form-control @error('seo.meta_keywords') error @enderror ignore-navigate-away" name="seo[meta_keywords]" data-role="tagsinput" placeholder="Meta Keywords" required autocomplete="off" value="{{ $blog->seo->meta_keywords ?? old('seo.meta_keywords') }}">
                            ...
                            <div class="form-group form-group-default @error('seo.image_alt') has-error @enderror">
                                ...
                                <input type="text" class="form-control @error('seo.image_alt') error @enderror" name="seo[image_alt]" placeholder="Meta Image Alt" autocomplete="off" value="{{ $blog->seo->image_alt ?? old('seo.image_alt') }}">
                                @error('seo.image_alt')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
                            <div class="row">
                                <div class="col-sm-12 col-md-12">
                                    <button type="button" class="btn btn-link btn-link-fix p-l-10 p-r-10 btn-lg open-use-media" data-type-media="single" data-populate-value=".populate-value-seo" data-populate-media=".populate-container-seo" id="{{ indexing() }}"><i class="pg-icon m-r-5">image</i> Browse Image</button>
                            ...
                            <div class="row m-t-10">
                                <div class="col-sm-12 col-md-12 col-lg-12 col-xlg-12">
                                    <input type="hidden" name="seo[meta_image_id]" class="populate-value-seo" value="{{ $blog->seo->image_id ?? 0 }}">
                                    <div class="populate-container populate-container-seo">
                                        @if (isset($blog->seo->image_id) && isset($blog->seo->image->image))
                                        <div class="populate-media">
                                            <a href="{{ secure_img($blog->seo->image->image, '1200') }}" target="_blank">
                                                <img src="{{ secure_img($blog->seo->image->image, '480X320') }}" class="full-width">
                                            </a>
                                        </div>
                                        @endif
                                    </div>
