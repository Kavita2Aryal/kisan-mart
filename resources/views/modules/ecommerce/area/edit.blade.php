...
<div class="container-fluid">
    <form class="m-t-20" role="form" method="POST" action="{{ route('area.update', [$area->uuid]) }}">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-6 col-xlg-6">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update Area
                    ...
                    <div class="card-body">
                        <div class="form-group form-group-default required @error('name') has-error @enderror">
                            <label>Name</label>
                            <div class="controls">
                                <input type="text" class="form-control @error('name') error @enderror" name="name" autocomplete="off" value="{{ $area->name ?? old('name') }}">
                                @error('name')
                        ...
                        <div class="form-group form-group-default form-group-default-select2 required @error('city_id') has-error @enderror">
                            ...
                            <select class="full-width @error('city_id') error @enderror" name="city_id" data-placeholder="Select a City" data-init-plugin="select2" required>
                                ...
                                <option value="{{ $row->id }}" {{ ($row->id == $area->city_id) ? 'selected' : '' }}>{{ $row->name }}</option>
                                @empty
                                @endforelse
                            </select>
                        ...
                        <div class="form-group form-group-default @error('delivery_charge') has-error @enderror">
                            ...
                            <div class="controls">
                                <input type="text" class="form-control custom-decimal-field @error('delivery_charge') error @enderror" name="delivery_charge" autocomplete="off" value="{{ $area->delivery_charge ?? old('delivery_charge') }}">
                        ...
                        <div class="form-group form-group-default @error('condition_amount') has-error @enderror">
                            ...
                            <div class="controls">
                                <input type="text" class="form-control custom-decimal-field @error('condition_amount') error @enderror" name="condition_amount" autocomplete="off" value="{{ $area->condition_amount ?? old('condition_amount') }}">
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="form-check info m-t-0 m-b-0">
                                    <input type="checkbox" name="is_active" value="10" id="checkbox-active" @if($area->is_active == 10) checked @endif>
                                    <label for="checkbox-active">Publish ?</label>
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6 col-lg-6 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    UPDATE <span class="visible-x-inline m-l-5">AREA</span>
                                </button>
                            </div>
                        </div>
                    </div>
