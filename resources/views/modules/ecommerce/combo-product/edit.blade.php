...
@section('title', 'Update Combo Product')

@section('content')
@php $currency = 'NPR'; @endphp

<div class="container-fluid">
    <form class="m-t-20" class="form-product" method="POST" action="{{ route('combo.product.update', [$product->uuid]) }}" data-hyperlink-search="{{ route('web.alias.hyperlink.search', [$product->uuid]) }}">
        @csrf
        @method('PUT')
        <!-- Product Main Form -->
        <div class="row">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">
                            ...
                            <button class="btn btn-link btn-link-fix p-l-10 p-r-10 m-r-5 pull-right" type="submit">
                                UPDATE <span class="visible-x-inline m-l-5">COMBO PRODUCT</span>
                            </button>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group form-group-default p-b-10 required @error('short_description') has-error @enderror">
                            ...
                            <textarea name="short_description" class="form-control @error('short_description') error @enderror" required style="height:50px;">{!! $product->short_description ?? old('short_description') !!}</textarea>
                        </div>
                                        ...
                                        <table class="table table-condensed">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="v-align-middle p-l-5 p-r-5">
                                                        <input name="qty" placeholder="QTY" type="number" min="0" max="9999999999" required class="form-control @error('qty') error @enderror" autocomplete="off" value="{{ $product->default_variant->qty ?? old('qty') }}">
                                                        @error('qty')
                                                        <label class="error">{{ $message }}</label>
                                                        @enderror
                                                    </td>
                ...
                <div class="card">
                    ...
                    <div class="card-body no-scroll no-padding">
                        ...
                        <div id="gallery-images">
                            ...
                            @if (Storage::exists('public/ecommerce/product/'.$gallery->image))
