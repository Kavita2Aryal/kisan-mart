...
<script src="{{ asset('assets/js/offer.min.js') }}" type="text/javascript"></script>
<script>
    summernote_init();
        ...
        init: function() {
            ...
            this.on("success", function(file, response) {
                if (response.status == 'success') {
                    $('.offer-image').val(response.filename)
    ...
    @if(isset($offer) && $offer->image != '' && Storage::exists('public/ecommerce/'.$offer->image))
    var mockFile = {
        name: "{{ $offer->image }}",
        size: "{{ \Storage::size('public/ecommerce/'.$offer->image) }}"
    };
    myDropzone.emit("addedfile", mockFile);
    myDropzone.emit("thumbnail", mockFile, "{{ \Storage::url('public/ecommerce/'.$offer->image) }}");
