...
<div class="container-fluid">
    <form class="m-t-20" id="form" role="form" method="POST" action="{{ route('policy.update', [$policy->uuid]) }}">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-7 col-xlg-7">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update Policy
                    ...
                    <div class="card-body">
                        <div class="form-group required form-group-default @error('title') has-error @enderror">
                            <label>Title</label>
                            <div class="controls">
                                <input name="title" type="text" class="form-control @error('title') error @enderror" autocomplete="off" value="{{ $policy->title ?? old('title') }}">
                                @error('title')
                        ...
                        <div class="form-group editor-description @error('description') has-error @enderror" data-index="{{ $indexing }}">
                            <textarea class="editor-container editor-container-{{ $indexing }} @error('description') error @enderror" name="description">{{ $policy->description ?? old('description') }}</textarea>
                            @error('description')
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="form-group form-group-default input-group @error('effective_date') has-error @enderror required">
                                    <div class="form-input-group">
                                        ...
                                        <input type="text" class="form-control effective-date @error('effective_date') error @enderror" name="effective_date" placeholder="Pick a date" autocomplete="off" value="{{ $policy->effective_date ?? old('effective_date') }}" data-provide="datepicker-inline">
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <div class="form-check info m-t-0 m-b-0">
                                    <input type="checkbox" name="is_active" value="10" id="checkbox-active" @if($policy->is_active == 10) checked @endif>
                                    <label for="checkbox-active">Publish ?</label>
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6 col-lg-6 text-right">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10" type="submit">
                                    UPDATE <span class="visible-x-inline m-l-5">POLICY</span>
                                </button>
                            </div>
                        </div>
                    </div>
