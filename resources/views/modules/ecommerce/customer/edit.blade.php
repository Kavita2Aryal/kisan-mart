...
<div class="container-fluid">
    <form role="form" class="m-t-20" method="POST" action="{{ route('customer.update', $customer->uuid) }}">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-sm-12 col-md-12 col-lg-4 col-xlg-4">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title full-width">Update Customer
                    ...
                    <div class="card-body">
                        <div class="form-group required form-group-default @error('name') has-error @enderror">
                            <label>Name</label>
                            <input type="text" class="form-control @error('name') error @enderror" name="name" required autocomplete="off" autofocus value="{{ $customer->name ?? old('name') }}">
                            @error('name')
                        ...
                        <div class="form-group form-group-default @error('email') has-error @enderror">
                            <label>Email</label>
                            <input type="email" class="form-control @error('email') error @enderror" name="email" autocomplete="off" value="{{ $customer->email ?? old('email') }}">
                            @error('email')
                            <label class="error">{{ $message }}</label>
                            @enderror
                        </div>
                        <div class="form-group form-group-default @error('phone') has-error @enderror">
                            <label>Phone</label>
                            <input type="text" class="form-control @error('phone') error @enderror" name="phone" autocomplete="off" value="{{ $customer->phone ?? old('phone') }}">
                            @error('phone')
                            <label class="error">{{ $message }}</label>
                            @enderror
                        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-12 m-t-10">
                                <button class="btn btn-link btn-link-fix p-l-10 p-r-10 btn-submit pull-right" type="submit">
                                    UPDATE <span class="visible-x-inline m-l-5">CUSTOMER</span>
                                </button>
                            </div>
                        </div>  
                    </div>
