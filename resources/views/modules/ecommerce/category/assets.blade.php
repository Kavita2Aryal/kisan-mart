...
<script>
    ...
    $(document).ready(function() {
        $('.category-prod').select2({});
    });
    $('.product-lists').select2({
        placeholder: 'Assign Products',
        ajax: {
            url: "{{ route('category.search', [$uuid]) }}",
            dataType: 'json',
            delay: 250,
        ...
        init: function() {
            ...
            this.on("success", function(file, response) {
                if (response.status == 'success') {
                    $('.category-image').val(response.filename)
    ...
    @if(isset($category) && $category->image != '' && Storage::exists('public/ecommerce/'.$category->image))
    var mockFile = {
        name: "{{ $category->image }}",
        size: "{{ \Storage::size('public/ecommerce/'.$category->image) }}"
    };
    myDropzone.emit("addedfile", mockFile);
    myDropzone.emit("thumbnail", mockFile, "{{ \Storage::url('public/ecommerce/'.$category->image) }}");
