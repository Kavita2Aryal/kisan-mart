...
<div class="container-fluid page-management">
    <div class="row m-t-20">
        <div class="col-sm-12 col-md-4 col-lg-4 col-xlg-3 m-b-20">
            ...
            @php $section = $sections[$sec['index']]; @endphp
        ...
        <div class="col-sm-12 col-md-8 col-lg-8 col-xlg-9">
            <div class="content-form content-active">
                <form role="form" method="POST" action="{{ route('page.store') }}" id="form-has-alias" data-alias="{{ route('web.alias.check') }}">
                    @csrf
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title full-width">Page Details
                                ...
                                <a href="{{ route('page.layout.create') }}" class="normal btn btn-link pull-right" data-tippy-content="Go Back" data-tippy-placement="left"><i class="pg-icon">arrow_left</i></a>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <input type="hidden" name="sections[{{ $index }}][index]" value="{{ $sec['index'] }}">
                            <input type="hidden" name="sections[{{ $index }}][display_order]" value="{{ $sec['display_order'] }}">
                            @empty
                            @endforelse
                        </div>
                    </div>
