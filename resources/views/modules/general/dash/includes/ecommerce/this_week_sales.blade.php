<div class="card bg-complete">
    <div class="card-header">
        <div class="card-title hint-text">
            <span class="font-montserrat fs-11 all-caps text-white">This Week Sales</span>
        </div>
    </div>
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-sm-12 col-md-8 col-lg-9 col-xlg-10 full-width-1024">
                @foreach($this_week_total_sales as $key => $row)
                <h4 class="no-margin p-b-5 text-white">{{ $key }}. {{ number_format($row, 2) }}</h4>
                @endforeach
                <!-- @if($last_week_total_sales == 0 && $this_week_total_sales == 0)
                <span class="label font-montserrat">No Transaction</span>
                @elseif($weekly_diff > 0)
                <span class="label font-montserrat">{{ round($weekly_percent) }}% Lower than last week</span>
                @else
                <span class="label font-montserrat">{{ round($weekly_percent) }}% Higher than last week</span>
                @endif -->
            </div>
        </div>
    </div>
