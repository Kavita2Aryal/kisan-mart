	...
	Flashy.prototype = {
		...
		_loadVideo: function(def, item, href) {
			...
			if (player) {
				...
				def.resolve();
			} else if (videoObj.type == 'user-manual') {
				this.controls.$content.html('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen frameborder="0" src="' + href + '"></iframe>');
				def.resolve();
			} else  {
				def.reject();
			}
		...
		_parseVideoUrl: function(url) {
			...
			if (RegExp.$3.indexOf('youtu') > -1) {
				...
				type = 'vimeo';
			} else if (url.indexOf('user-manual') > -1) {
				type = 'user-manual';
			}
