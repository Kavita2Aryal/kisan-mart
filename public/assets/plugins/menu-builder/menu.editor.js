(function($) {
    $.fn.sortableLists = function(options) {
        var jQBody = $('body').css('position', 'relative'),
            ...
            opener = $('<span/>').addClass('sortableListsOpener ' + setting.opener.openerClass).css(setting.opener.openerCss).on('mousedown touchstart', function(e) {
        ...
        function startDrag(e, el, rEl) {
            ...
            var elMT = parseInt(el.css('margin-top')),
            ...
            var placeholderNode = state.placeholderNode = $('#sortableListsPlaceholder');
            el.css({
                'width': el.width(),
                'position': 'absolute',
                'top': elXY.top - elMT,
            ...
            state.doc.on('mousemove touchmove', dragging).on('mouseup touchend touchcancel', endDrag);
        }
        ...
        function dragging(e) {
            if (state.isDragged) {
                ...
                cEl.el[0].style.visibility = 'hidden';
        ...
        function endDrag(e) {
            ...
            cEl.el.animate({
                ...
                hintNode.remove();
                hintWrapperNode.removeAttr('id').removeClass(setting.hintWrapperClass);
            ...
            scrollStop(state);
            state.doc.unbind("mousemove touchmove", dragging).unbind("mouseup touchend touchcancel", endDrag);
        }
        ...
        function elFromPoint(x, y) {
            ...
            if (isRelEFP === null) {
                ...
                if ((s = state.doc.scrollTop()) > 0) {
                    isRelEFP = ((res = document.elementFromPoint(0, s + $(window).height() - 1)) == null || res.tagName.toUpperCase() == 'HTML');
                }
                if ((s = state.doc.scrollLeft()) > 0) {
                    isRelEFP = ((res = document.elementFromPoint(s + $(window).width() - 1, 0)) == null || res.tagName.toUpperCase() == 'HTML');
                }
            }
            if (isRelEFP) {
            ...
            if (!state.rootEl.el.find(el).length) {
                return null;
            } else if (el.is('#sortableListsPlaceholder') || el.is('#sortableListsHint')) {
                return null;
            } else if (!el.is('li')) {
                el = el.closest('li');
                return el[0] ? el : null;
        ...
        function showOnBottom(e, oEl) {
            ...
            if (e.pageX - oEl.offset().left < setting.insertZone) {
                ...
                var children = oEl.children(),
                    list = oEl.children(setting.listSelector).last();
...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        listOptions: {
            ...
            opener: {
                as: 'html',
                close: '<i class="pg-icon">minus</i>',
                open: '<i class="pg-icon">plus</i>',
                openerCss: {
                    'margin-right': '10px',
                    'float': 'none'
                },
                openerClass: 'btn btn-default',
            },
            placeholderCss: {
    ...
    $main.on('click', '.btnIn', function(e) {
        ...
        if ($prev.length > 0) {
            ...
            if ($ul.length > 0) $ul.append($li);
                ...
                var $ul = $('<ul>').addClass('p-l-0').css('padding-top', '10px');
                $prev.append($ul);
                $ul.append($li);
    ...
    function editItem($item) {
        var data = $item.data(); console.log(data);
        $.each(data, function(p, v) {
            $form.find("[name=" + p + "]").val(v);
            if (p == 'type' && (v != 'external' && v != '' && typeof(v) !== 'undefined')) {
            ...
            if (p == 'image') {
                (v != '') ? $('.populate-container').show() : $('.populate-container').hide();
            }

            if (p == 'image_preview') {
                $('.populate-media').find('img').attr('src', v);
                $('.populate-media').find('a').attr('href', v);
            }
        });
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({
            classCss: 'btn btn-default btnEdit',
            text: '<i class="pg-icon clickable">pencil</i>'
        });
        var $btnRemv = TButton({
            classCss: 'btn btn-default btnRemove',
            text: '<i class="pg-icon clickable">trash_alt</i>'
        });
        var $btnUp = TButton({
            classCss: 'btn btn-default btnUp btnMove',
            text: '<i class="pg-icon clickable">chevron_up</i>'
        });
        var $btnDown = TButton({
            classCss: 'btn btn-default btnDown btnMove',
            text: '<i class="pg-icon clickable">chevron_down</i>'
        });
        var $btnOut = TButton({
            classCss: 'btn btn-default btnOut btnMove',
            text: '<i class="pg-icon clickable">arrow_down</i>'
        });
        var $btnIn = TButton({
            classCss: 'btn btn-default btnIn btnMove',
            text: '<i class="pg-icon clickable">arrow_up</i>'
        });
    ...
    function createMenu(arrayItem, depth) {
        var level = (typeof(depth) === 'undefined') ? 0 : depth;
        var $elem = (level === 0) ? $main : $('<ul>').addClass('p-l-0').css('padding-top', '10px');
    ...
    function TOpener(li) {
        var opener = $('<span>').addClass('sortableListsOpener ' + options.opener.openerClass).css(options.opener.openerCss).on('mousedown touchstart', function(e) {
            var li = $(this).closest('li');
            if (li.hasClass('sortableListsClosed')) {
                li.iconOpen(options);
            } else {
    ...
    this.setInputURL = function($ipnut) {
        $inputURL = $ipnut;
    };
    ...
    this.update = function() {
        ...
        $form.find('.item-menu').each(function() {
            $cEl.data($(this).attr('name'), $(this).val().replace('\t',''));
        });
    ...
    this.add = function() {
        ...
        var div = $('<div>').css({ "overflow": "auto" }).append(textItem).append(btnGroup);
        var $li = $("<li>").data(data);
